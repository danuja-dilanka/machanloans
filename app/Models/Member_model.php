...
class Member_model extends Model {
    ...
    public function tranfer_unreg_to_reg_mem($id = 0) {
        ...
        if (isset($unreg_mem->id)) {
            ...
            $ins_id = $this->add_unreg_member([
                ...
                "cred_address" => $data["cred_address"],
                ...
                "gender" => $data["gender"],
                "city" => $data["city"],
                'rel_friend1' => $data["rel_friend1"],
                'rel_friend1_phone' => $data["rel_friend1_phone"],
                'rel_friend1_address' => $data["rel_friend1_address"],
                'rel_friend2' => $data["rel_friend2"],
                'rel_friend2_phone' => $data["rel_friend2_phone"],
                'rel_friend2_address' => $data["rel_friend2_address"],
    ...
    public function get_unreg_mem_data($id = 0, $result_type = 0) {
        $result = $this->db->table(DB_PREFIX . 'unreg_member a');
        $result->select('a.*,b.email AS login_email, CONCAT(a.first_name, " ", a.last_name) AS full_name');
        $result->join(DB_PREFIX . 'user b', 'a.id = b.rel_id', 'left');
        if ($id > 0) {
            return $result->where(["a.id" => $id])->get()->getRow();
