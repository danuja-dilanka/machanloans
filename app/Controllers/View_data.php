...
//                (has_permission("user_role", "delete") ? "<a href='#' data-id='" . base_url("setting/del_user_role/") . $key_enc . "' class='btn btn-sm btn-danger confirm_red_btn'>Delete</a>" : "")
    ...
    public function net_profit_report() {
        ...
        if ($date_from != "" && $date_to != "") {
            $loan_pays = $loan_model->get_loan_pay_all_data_by("a.pay_date >= '$date_from' AND a.pay_date =< '$date_to'", "SUM(a.total) AS total_amount, a.pay_date", "pay_date");
        } else {
        ...
        if ($date_from != "" && $date_to != "") {
            $loan_reles = $loan_model->get_loan_release_by("a.rel_date >= '$date_from' AND a.rel_date =< '$date_to'", "SUM(c.last_amount) AS total_amount, rel_date", "rel_date");
        } else {
